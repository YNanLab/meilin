...
cat("Running DE for epithelial cells: early vs pre\n")
mark <- FindMarkers(epithelialcell, ident.1 = "SSc_postCART_early",ident.2="SSc_preCART") %>%
  arrange(desc(avg_log2FC))
rank <- setNames(mark$avg_log2FC, rownames(mark))
fgsea_res <- fgsea(bader_net, stats = rank, minSize = 15, maxSize = 500) %>%
  as_tibble() %>%
  arrange(padj) %>%
  mutate(comparison =  "1m FU vs BL", celltype = "epidermal cell")
...
saveRDS(fgsea_res, paste0('/Users/xu/Desktop/1_project/fgsea/data/20250430_ErlCARTXenium_epidermal_earlyvspre_FgseaRes_', tstamp, '.rds'))
