---
title: " suprabasal superlatevspre fgsea"
author: "Yi-Nan Li"
date: "`r Sys.Date()`"
output: 
   html_document:
...
cat("Running DE for suprabasal cells: superlate vs pre\n")
mark <- FindMarkers(suprabasal, ident.1 = "SSc_postCART_superlate", ident.2 = "SSc_preCART") %>%
  arrange(desc(avg_log2FC))
rank <- setNames(mark$avg_log2FC, rownames(mark))
fgsea_res <- fgsea(bader_net, stats = rank, minSize = 15, maxSize = 500) %>%
  as_tibble() %>%
  arrange(padj) %>%
  mutate(comparison = "superlate_vs_pre", celltype = "suprabasal cell")
...
saveRDS(fgsea_res, paste0('/Users/xu/Desktop/1_project/fgsea/data/20250416_ErlCARTXenium_supra_superlatevspre_FgseaRes_', tstamp, '.rds'))
