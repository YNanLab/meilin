...
epibasal<- obj[,obj$epithelialsubtype2== 'epi basal cell']
rm(obj)
gc()
...
bader_net <- filter(bader_net, gene %in% rownames(epibasal))
...
Idents(epibasal) <- epibasal$fourgroups


cat("Running DE for epibasal cells: superlate vs pre\n")
mark <- FindMarkers(epibasal, ident.1 = "SSc_postCART_superlate", ident.2 = "SSc_preCART") %>%
  arrange(desc(avg_log2FC))
rank <- setNames(mark$avg_log2FC, rownames(mark))
fgsea_res <- fgsea(bader_net, stats = rank, minSize = 15, maxSize = 500) %>%
  as_tibble() %>%
  arrange(padj) %>%
  mutate(comparison = "12m FU vs BL", celltype = "epibasal cell")
...
saveRDS(fgsea_res, paste0('/Users/xu/Desktop/1_project/fgsea/data/20250416_ErlCARTXenium_epibasal_superlatevspre_FgseaRes_', tstamp, '.rds'))
...
Rscript -e "rmarkdown::render('script/epibasal_superlate_pre_fgsea.Rmd', output_file = 'html/20250416_ErlCART_Xenium_epibasal_superlatevspre_FgseaRes_$(date +"%Y%m%d_%H%M%S").html')" > html/logs/fgsea_$(date +"%Y%m%d_%H%M%S").log 2>&1
```
