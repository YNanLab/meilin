...
cat("Running DE for epibasal cells: early vs pre\n")
mark <- FindMarkers(epibasal, ident.1 = "SSc_postCART_early", ident.2 = "SSc_preCART") %>%
  arrange(desc(avg_log2FC))
rank <- setNames(mark$avg_log2FC, rownames(mark))
fgsea_res <- fgsea(bader_net, stats = rank, minSize = 15, maxSize = 500) %>%
  as_tibble() %>%
  arrange(padj) %>%
  mutate(comparison = "1m FU vs BL", celltype = "epibasal cell")
...
saveRDS(fgsea_res, paste0('/Users/xu/Desktop/1_project/fgsea/data/20250416_ErlCARTXenium_epibasal_earlyvspre_FgseaRes_', tstamp, '.rds'))
...
Rscript -e "rmarkdown::render('script/2.epibasal early vs pre.Rmd', output_file = 'html/20250416_ErlCART_Xenium_epibasal_early_pre_FgseaRes_$(date +"%Y%m%d_%H%M%S").html')" > html/logs/fgsea_$(date +"%Y%m%d_%H%M%S").log 2>&1
```
